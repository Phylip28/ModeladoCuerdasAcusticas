...
def _extraer_ecuacion_polinomial(modelo_pipeline, grado: int) -> str:
    ...
    for i, c in enumerate(coefs, start=1):
        ...
        valor = abs(c)
        terminos.append(
            f"{signo} {valor:.6g}·x" if i == 1 else f"{signo} {valor:.6g}·x^{i}"
        )
...
def _agregar_tabla_errores(
    pdf: _PDFReporte, filas: Sequence[tuple[str, float, str]]
) -> None:
    pdf.add_page()
    ...
    for idx, (nombre, mse, r2_str) in enumerate(filas):
        pdf.set_fill_color(*(_COLOR_FILA_PAR if idx % 2 == 0 else _COLOR_BLANCO))
...
def _agregar_ecuaciones(
    pdf: _PDFReporte, ecuaciones: Sequence[tuple[str, str]]
) -> None:
    pdf.ln(10)
    pdf.set_font(_FONT_FAMILY, "B", 14)
    pdf.set_text_color(*_COLOR_ENCABEZADO)
    pdf.cell(
        0, 10, "Ecuaciones de los Modelos Polinomiales", new_x="LMARGIN", new_y="NEXT"
    )
    pdf.ln(4)
    pdf.set_text_color(0, 0, 0)
...
def _agregar_grafico(pdf: _PDFReporte, ruta_grafico: Path) -> None:
    ...
    pdf.ln(4)
    pdf.image(str(ruta_grafico), x=pdf.l_margin, w=pdf.w - pdf.l_margin - pdf.r_margin)


def _generar_grafico_loss_curve(loss_curve: list[float], directorio: Path) -> Path:
...
def _agregar_loss_curve(pdf: _PDFReporte, ruta_loss: Path) -> None:
    ...
    pdf.ln(4)
    pdf.image(str(ruta_loss), x=pdf.l_margin, w=pdf.w - pdf.l_margin - pdf.r_margin)
...
def generar_pdf_reporte(
    ...
    for modelo in datos_modelos:
        ...
        if modelo.get("pipeline") is not None and modelo.get("grado") is not None:
            ecuaciones.append(
                (
                    modelo["nombre"],
                    _extraer_ecuacion_polinomial(modelo["pipeline"], modelo["grado"]),
                )
            )
    ...
    if loss_curve:
        _agregar_loss_curve(
            pdf, _generar_grafico_loss_curve(loss_curve, directorio_salida)
        )
